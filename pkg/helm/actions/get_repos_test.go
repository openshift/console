...
entries:
  ...
  - apiVersion: v2
    ...
    description: |-
      ...
      By installing this product you accept the license terms http://ibm.biz/oms-license and http://ibm.biz/oms-apps-license.
    digest: e1bd6eedeedbd07d38a610bb79550d2815555f444493f6d8331d395212813772
    icon: https://raw.githubusercontent.com/IBM/charts/master/logo/ibm-oms-logo.png
    keywords:
    - oms
    - sterling
  ...
  - apiVersion: v2
    appVersion: 1.16.0
    created: "2020-07-15T16:32:34.698385506+05:30"
    description: A Helm chart for Kubernetes
...
func TestHelmConfigBuilder_IndexFiles(t *testing.T) {
	tests := []struct {
		testName        string
		indexFile       string
		expectedEntries int
	}{{
		testName:        "correct index.yaml should report correct no of chart entries",
		indexFile:       testdata.AzureRepoYaml,
		expectedEntries: 14,
	}}
	...
	for i, tt := range tests {
		...
		if len(indexFile.Entries) != tt.expectedEntries {
			t.Errorf("index length mismatch expected is %d received %d", len(indexFile.Entries), tt.expectedEntries)
		}
	}
}

func TestHelmConfigBuilder_ClusterConfigs(t *testing.T) {
	tests := []struct {
		testName        string
		response        []string
		expectedEntries int
		...
		{
			testName:        "with both repos",
			response:        []string{sampleRepoYaml, azureRepoYaml},
			expectedEntries: 20,
			repoName:        []string{"redhat-repo", "azure-repo"},
		},
		{
			testName:        "with single repos",
			response:        []string{azureRepoYaml},
			expectedEntries: 14,
			repoName:        []string{"azure-repo"},
		},
		{
			testName:        "with no repo configured",
			response:        []string{},
			expectedEntries: 6,
			repoName:        []string{},
		},
	...
	for _, test := range tests {
		client := fake.SetupClusterWithHelmCrs(test.response)
		file, err := FetchIndexFile(client, nil, nil)
		if err != nil {
			t.Error(err)
		}
		if len(file.Entries) != test.expectedEntries {
			t.Errorf("Total chart entries are not matching expected is %d and received %d", test.expectedEntries, len(file.Entries))
		}
	}
}

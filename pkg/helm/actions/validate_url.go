...
func isInternalChartHost(hostname string) bool {
	...
	for _, exact := range internalChartHostExact {
		if h == exact {
			return true
		}
	}
	for _, suffix := range internalChartHostSuffixes {
...
// ValidateChartURL exports the SSRF guard for call sites outside the actions
// package (e.g. the chart-verifier handler). It delegates to the validateChartURL
// seam so the test suite relaxes it uniformly.

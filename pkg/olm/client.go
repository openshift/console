...
type CatalogdClientInterface interface {
	Fetch(catalog, baseUrl string) ([]*declcfg.Package, []*declcfg.Bundle, error)
}
...
func (c *CatalogdClient) Fetch(catalog, baseUrl string) ([]*declcfg.Package, []*declcfg.Bundle, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to fetch catalog %s from %s: %w", catalog, catalogURL, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, nil, fmt.Errorf("failed to fetch catalog %s from %s: status code %d", catalog, catalogURL, resp.StatusCode)
	}

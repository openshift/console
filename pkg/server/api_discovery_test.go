...
func TestApiDiscoveryHandler_ForwardsAuthHeaders(t *testing.T) {
	...
	req.Header.Add("Impersonate-Group", "group-a")
	req.Header.Add("Impersonate-Group", "group-b")
	...
	if len(capturedGroups) != 2 || capturedGroups[0] != "group-a" || capturedGroups[1] != "group-b" {
		t.Errorf("expected Impersonate-Group headers forwarded, got %v", capturedGroups)
	}
}

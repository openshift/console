...
func withTokenReview(authenticator auth.Authenticator, h http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		user, err := authenticator.Authenticate(w, r)
		if err != nil {
			klog.Errorf("TOKEN_REVIEW: '%s %s' unauthorized, unable to authenticate, %v", r.Method, r.URL.Path, err)
			w.WriteHeader(http.StatusUnauthorized)
			return
		}
	...
	return http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			if authorizationHeader == "" {
				klog.Errorf("TOKEN_REVIEW: '%s %s' unauthorized, missing Authorization header", r.Method, r.URL.Path)
				w.WriteHeader(http.StatusUnauthorized)
				return
			}

			if !strings.HasPrefix(authorizationHeader, "Bearer") {
				klog.Errorf("TOKEN_REVIEW: '%s %s' unauthorized, 'Bearer' type Authorization header required", r.Method, r.URL.Path)
				w.WriteHeader(http.StatusUnauthorized)
				return
			}
			...
			if bearerToken == "" {
				klog.Errorf("TOKEN_REVIEW: '%s %s' unauthorized, empty or missing Bearer token", r.Method, r.URL.Path)
				w.WriteHeader(http.StatusUnauthorized)
				return
			}

...
var map_ConfigMapFileReference = map[string]string{
	...
	"key":  "key is the logo key inside the referenced ConfigMap. Must consist only of alphanumeric characters, dashes (-), underscores (_), and periods (.). Must be at most 253 characters in length. Must end in a valid file extension. A valid file extension must consist of a dot ('.') followed by 2 to 5 alpha characters. This validation rule ensures that the ConfigMap key ends with a valid file extension consisting of 2 to 5 letters.",
}
...
var map_Logo = map[string]string{
	...
	"themes": "themes specifies the themes for the console UI logo. themes is a required field that allows a list of themes. Each item in the themes list must have a unique mode and a source field. Each mode determines whether the logo is for the dark or light mode of the console UI. There must be at least one entry and no more than 2 entries.",
}

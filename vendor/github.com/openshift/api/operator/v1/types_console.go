...
type ConfigMapFileReference struct {
	...
	// A valid file extension must consist of a dot ('.') followed by 2 to 5 alpha characters.
	// +kubebuilder:validation:MaxLength=253
	// This validation rule ensures that the ConfigMap key ends with a valid file extension consisting of 2 to 5 letters.
...
// This validation rule ensures that the configMap field is set when the from field is 'ConfigMap'.

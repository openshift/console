...
func TestCreateHandler_ExistingCluster(t *testing.T) {
	store := NewStore()
	store.AddCluster("bootkube", validBootkubeCluster())

	h := createHandler(store, false)
	data, err := validBootkubeCreateJSON()
	assert.Nil(t, err)
	req, _ := http.NewRequest("POST", "/", data)
	w := httptest.NewRecorder()
	h.ServeHTTP(context.Background(), w, req)
	assert.Equal(t, http.StatusConflict, w.Code)
	assert.Equal(t, "already created a cluster\n", w.Body.String())
}

func validBootkubeCreate() (*CreateOperation, error) {
	...
	return &CreateOperation{
		BootcfgRPC:        "bootcfg.example.com:8081",
...
func validBootkubeCreateJSON() (io.Reader, error) {
	data, err := validBootkubeCreate()
	if err != nil {
		return nil, err
	}
	buf := &bytes.Buffer{}

...
func etcdInitialCluster(nodes []Node) string {
	...
	for i, node := range nodes {
		peers[i] = fmt.Sprintf("%s=http://%s:2380", etcdNodeName(i), node.IP.String())
	}
	return strings.Join(peers, ",")
}

// etcdEndpoints returns the comma separated list of etcd client urls which
// can be polled to determine availability of the etcd cluster with the
// given nodes.
func etcdEndpoints(nodes []Node) string {
	peers := make([]string, len(nodes))
	for i, node := range nodes {
		peers[i] = fmt.Sprintf("http://%s:2379", node.IP.String())
	}
	return strings.Join(peers, ",")
}
...
func etcdURLs(nodes []Node) ([]*url.URL, error) {
	...
	for i, node := range nodes {
		raw := fmt.Sprintf("http://%s:2379", node.IP.String())
		u, err := url.Parse(raw)
		if err != nil {
			return nil, err
		}

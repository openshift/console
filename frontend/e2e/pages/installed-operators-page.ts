...
export class InstalledOperatorsPage extends BasePage {
  ...
  async navigateToInstalledOperators(): Promise<void> {
    await this.goTo('/k8s/all-namespaces/operators.coreos.com~v1alpha1~ClusterServiceVersion');
  }
  ...
  getOperatorRow(operatorName: string): Locator {
    return this.page
      .locator('tr')
      .filter({ has: this.page.getByTestId(`operator-row-${operatorName}`) });
  }
   ...
   */
  async verifyOperatorInstallationSucceeded(
    operatorName: string,
    namespace?: string,
  ): Promise<void> {
  ...
  async navigateToOperatorDetails(
    operatorName: string,
    operatorURLName: string,
    namespace: string = 'openshift-operators',
  ): Promise<void> {
    await this.navigateToInstalledOperators();
    ...
    await this.goTo(
      href ??
        `/k8s/ns/${namespace}/operators.coreos.com~v1alpha1~ClusterServiceVersion/${operatorURLName}`,
    );
   ...
   */
  async verifyOperatorNotInstalledInNamespace(
    operatorName: string,
    namespace: string,
  ): Promise<void> {
    await this.navigateToInstalledOperators();
  ...
  async selectNamespace(namespace: string): Promise<void> {
    const namespaceDropdownButton = this.page
      .getByTestId('namespace-bar-dropdown')
      .getByRole('button')
      .first();
    await this.robustClick(namespaceDropdownButton);

    const textFilter = this.page.getByTestId('dropdown-text-filter');
    await expect(textFilter).toBeVisible({ timeout: 30_000 });

...
export class OperatorInstallPage extends BasePage {
  ...
  private readonly operatorRecommendedRadio = this.page.getByTestId(
    'Operator recommended Namespace:-radio-input',
  );
  ...
  private readonly viewInstalledOperatorsBtn = this.page.getByTestId(
    'view-installed-operators-btn',
  );
  ...
  async installOperatorGlobally(operatorName: string, operatorCardTestID: string): Promise<void> {
    ...
    await expect(this.viewInstalledOperatorsBtn).toContainText(

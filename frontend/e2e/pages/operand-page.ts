...
export class OperandPage extends BasePage {
  ...
  private readonly operandDetailsSection = this.page
    .getByTestId('operand-details__section--info')
    .first();
  ...
  getTagItemContent(fieldId: string): Locator {
    return this.page.locator(`[id="${quoteAttributeValue(`${fieldId}_field`)}"] .tag-item-content`);
  }

...
export class OperatorDetailsPage extends BasePage {
  ...
  async verifyDetailsPageSections(): Promise<void> {
    ...
    await expect(this.getSectionHeading('ClusterServiceVersion details')).toBeVisible({
      timeout: 30_000,
    });
  ...
  async navigateToOperandTab(operandName: string, isGlobal: boolean = true): Promise<void> {
    ...
    if (isGlobal) {
      ...
      await expect(this.page.getByTestId('horizontal-link-All instances')).toBeVisible({
        timeout: 5_000,
      });
  ...
  async deleteOperand(testOperand: TestOperandProps, isGlobal: boolean = true): Promise<void> {
    ...
    await expect(this.page).toHaveURL((url) =>
      url.pathname.endsWith(`/${testOperand.exampleName}`),
    );
  ...
  async uninstallOperatorWithOperands(deleteOperands: boolean = false): Promise<void> {
    ...
    if (deleteOperands) {
      ...
      try {
        ...
        console.log(
          '⏹️ No delete-all-operands checkbox found - this operator may only have one operand',
        );
   ...
   */
  async uninstallOperatorWithAPIError(
    errorType: 'cannot-load-operands' | 'error-deleting-operands',
  ): Promise<void> {
  ...
  private async clickSubmitButton(): Promise<void> {
    const submitButton = this.page.locator(
      '[data-test="confirm-action"], .pf-v6-c-button.pf-m-primary[type="submit"]',
    );

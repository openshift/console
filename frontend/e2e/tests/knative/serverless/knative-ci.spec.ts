...
test.describe('Knative CI smoke tests', { tag: ['@smoke', '@regression'] }, () => {
  ...
  test('KN-01-TC12: Delete Revision not possible for single revision', async ({ page }) => {
    ...
    await test.step('Verify unable-to-delete modal', async () => {
      ...
      await expect(
        page.getByText('You cannot delete the last Revision for the Service.'),
      ).toBeVisible();
  ...
  test('Create Revision for existing knative Service', async ({ page }) => {
    ...
    await test.step('Edit service to create a new revision', async () => {
      await page.goto(
        `/edit/ns/${namespace}?name=${SERVICE_NAME}&kind=serving.knative.dev~v1~Service`,
      );
  ...
  test('KN-02-TC10: Set traffic distribution >100%', async ({ page }) => {
    ...
    await test.step('Set traffic >100% and verify error', async () => {
      await expect(page.getByText('Set traffic distribution', { exact: true })).toBeVisible({
        timeout: 30_000,
      });
  ...
  test('KE-05-TC11: Delete Broker', async ({ page }) => {
    ...
    await test.step('Delete broker via details page', async () => {
      await topologyPage.openServiceAction(
        namespace,
        'default-broker',
        'Delete Broker',
  ...
  test('KN-02-TC16: Delete service', async ({ page }) => {
    ...
    await test.step('Verify service removed', async () => {
      await page.reload();
      await expect(page.locator('[data-type="knative-service"]')).not.toBeAttached({
        timeout: 30_000,
      });
    });
  });

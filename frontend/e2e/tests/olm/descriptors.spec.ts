...
test.describe('Using OLM descriptor components', { tag: ['@admin'] }, () => {
  ...
  test('creates an operand using the form', async ({ page, cleanup }) => {
    ...
    await test.step('Verify select field', async () => {
      ...
      await expect(operandPage.getFormFieldInput(FIELD_IDS.SELECT)).toHaveText(testCR.spec.select);
    });
    ...
    await test.step('Verify field group', async () => {
      ...
      await expect(operandPage.getFormFieldLabel(`${FIELD_IDS.FIELD_GROUP}_itemOne`)).toHaveText(
        'itemOne',
      );
      await expect(operandPage.getFormFieldInput(`${FIELD_IDS.FIELD_GROUP}_itemOne`)).toHaveValue(
        testCR.spec.fieldGroup.itemOne,
      );
      await expect(operandPage.getFormFieldLabel(`${FIELD_IDS.FIELD_GROUP}_itemTwo`)).toHaveText(
        'itemTwo',
      );
      await expect(operandPage.getFormFieldInput(`${FIELD_IDS.FIELD_GROUP}_itemTwo`)).toHaveValue(
        String(testCR.spec.fieldGroup.itemTwo),
      );
    });

...
test.describe('Quick Starts - Developer Perspective', { tag: ['@dev-console'] }, () => {
  ...
  test('QS-03-TC01: Build with guided documentation card on Add page', async () => {
    test.skip(
      true,
      'Preserved skip: source scenario is tagged @broken-test in quick-start-devperspective.feature',
    );
  });
  ...
  test('QS-03-TC03: Quick start completed state shows Complete label', async ({
    ...
    await k8sClient.patchConfigMap('user-settings-kubeadmin', 'openshift-console-user-settings', {
      'console.quickstart.allStates': JSON.stringify({
        'sample-application': { status: 'Complete', taskNumber: 0 },
      }),
    });
    await warmupSPA(page);
    const quickStarts = new QuickStartsPage(page);
    await quickStarts.navigateToCatalog();
    const card = quickStarts.getQuickStartCard('sample-application');
    await expect(card).toBeVisible({ timeout: 30_000 });
    await expect(card).toContainText('Complete');
    await expect(card.getByText(/minutes/i)).toBeVisible();
  });

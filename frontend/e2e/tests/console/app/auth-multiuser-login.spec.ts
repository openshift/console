...
test.describe(
  'Auth test',
  {
    // Opt out of the page fixture's transparent OAuth re-auth. That recovery
    // re-authenticates as the *project's* persona -- admin for the `console`
    // project -- so it would log this spec in as kubeadmin the moment
    // performLogin's first navigation bounced to OAuth, and performLogin would
    // then early-return on the already-visible user menu without ever reaching
    // the htpasswd identity provider.
    annotation: { type: 'no-auto-reauth', description: 'asserts login as a specific user' },
  },
  () => {

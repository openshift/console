...
.provider('wsFactory', function() {
  ...
  this.$get = function($log, $window, $interval, $timeout) {
    ...
    function WebSocketWrapper(id, options) {
      ...
      if (this.options.bufferEnabled) {
        flushCanceler = $interval(this.flushBuffer.bind(this), this.options.bufferFlushInterval);
      }
    ...
    WebSocketWrapper.prototype._reconnect = function() {
      ...
      function attempt() {
        ...
        that._connectionAttempt = $timeout(attempt, delay);
        $log.log('attempting reconnect in', delay / 1000, 'seconds...');
      }
      this._connectionAttempt = $timeout(attempt, delay);
    };
    ...
    WebSocketWrapper.prototype._connect = function() {
      ...
      this.ws.onopen = function() {
        $log.log('websocket open: ', that.id);
        that._state = 'open';
      ...
      this.ws.onclose = function() {
        $log.log('websocket closed: ', that.id);
        that._state = 'closed';
      ...
      this.ws.onerror = function(code, reason) {
        $log.log('websocket error: ', that.id);
        that._state = 'error';
    ...
    WebSocketWrapper.prototype.destroy = function() {
      $log.log('websocket destroy: ', this.id);
      if (this._state === 'destroyed') {
        return;
      }
      this.ws.close();

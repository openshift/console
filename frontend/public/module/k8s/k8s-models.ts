...
k8sModels = k8sModels.withMutations(map => {
  const pluginModelDefinitions = pluginStore.getAlwaysOnExtensions().filter(isModelDefinition);
  const pluginModels = _.flatMap(pluginModelDefinitions.map(md => md.properties.models));

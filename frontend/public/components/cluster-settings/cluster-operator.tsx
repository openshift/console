...
export const ClusterOperatorList: React.FC<ClusterOperatorListProps> = ({
  ...
  const clusterOperatorStatusFilterOptions = React.useMemo<DataViewFilterOption[]>(() => {
    ...
    const statusCounts = data.reduce((counts, operator) => {
    ...
    return [
      {
        value: 'Available',
        label: `${t('public~Available')} (${statusCounts.Available || 0})`,
      },
      {
        value: 'Progressing',
        label: `${t('public~Progressing')} (${statusCounts.Progressing || 0})`,
      },
      {
        value: 'Degraded',
        label: `${t('public~Degraded')} (${statusCounts.Degraded || 0})`,
      },
      {
        value: 'Cannot update',
        label: `${t('public~Cannot update')} (${statusCounts['Cannot update'] || 0})`,
      },
      {
        value: 'Unavailable',
        label: `${t('public~Unavailable')} (${statusCounts.Unavailable || 0})`,
      },
      {
        value: 'Unknown',
  ...
  const additionalFilterNodes = React.useMemo<React.ReactNode[]>(
    ...
    [clusterOperatorStatusFilterOptions, t],
  );

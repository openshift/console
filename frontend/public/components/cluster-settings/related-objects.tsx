...
const RelatedObjects: React.FC<RelatedObjectsProps> = ({ data }) => {
  ...
  // TODO: elimintate this once Vojtech fixes ConsoleDataView
  const getResourceMetadata = React.useCallback(
    (obj: ClusterOperatorObjectReference) => ({
      name: obj.name,
    }),
    [],
  );
    ...
    <React.Suspense fallback={<div className="loading-skeleton--table" />}>
      <ConsoleDataView<ClusterOperatorObjectReference, RelatedObjectsRowData, RelatedObjectsFilters>
        ...
        getDataViewRows={(rowData, tableColumns) =>
          getRelatedObjectsDataViewRows(rowData, tableColumns)
        }

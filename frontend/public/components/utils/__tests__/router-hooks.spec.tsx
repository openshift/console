...
import { useQueryParamsMutator, useRouterPush } from '../router';
...
describe('useRouterPush', () => {
  it('should return a navigation function', () => {
    ...
    const { result } = renderHook(() => useRouterPush(), { wrapper });

    expect(typeof result.current).toBe('function');
  });

  it('should navigate to new URL when called', () => {
    const wrapper = ({ children }) => (
      <MemoryRouter initialEntries={['/test']}>
        <Routes>
          <Route path="/test" element={children} />
    ...
    // Navigation happens - we can't easily test the result without a full React tree
    // but we verify the function can be called without error

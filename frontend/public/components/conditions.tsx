...
export const Conditions: React.SFC<ConditionsProps> = ({ conditions, showReason = true, showMessage = true, }) => {
  const rows = conditions?.map?.((condition: K8sResourceCondition, i: number) => (
    <div className="row" data-test-id={condition.type} key={i}>
      ...
      {showReason && <div className="col-xs-4 col-sm-3 col-md-2">
        <CamelCaseWrap value={condition.reason} />
      </div>}
      {/* remove initial newline which appears in route messages */}
      {showMessage && <div className="hidden-xs col-sm-5 col-md-4 co-break-word co-pre-line co-conditions__message">
      ...
      {conditions?.length ? (
        <div className="co-m-table-grid co-m-table-grid--bordered">
          <div className="row co-m-table-grid__head">
            ...
            {showReason && <div className="col-xs-4 col-sm-3 col-md-2">Reason</div>}
            {showMessage && <div className="hidden-xs col-sm-5 col-md-4">Message</div>}
          </div>

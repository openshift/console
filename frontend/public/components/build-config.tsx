  ...
  const columns = React.useMemo(() => {
    return [
      ...
      },
      {
        ...
        sort: (data, direction) =>
          data.sort((a, b) => {
            const aDuration = displayDurationInWords(
              a?.latestBuild?.status?.startTimestamp,
              a?.latestBuild?.status?.completionTimestamp,
            );
            const bDuration = displayDurationInWords(
              b?.latestBuild?.status?.startTimestamp,
              b?.latestBuild?.status?.completionTimestamp,
            );

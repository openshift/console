...
import { BuildConfigModel, BuildModel, ChargebackReportModel, DeploymentConfigModel, GroupModel, ImageStreamModel, MachineAutoscalerModel, MachineConfigModel, MachineConfigPoolModel, MachineHealthCheckModel, MachineModel, MachineSetModel, UserModel } from '../../models';
...
const monitoringAlertsStartsWith = ['monitoring/alerts', 'monitoring/alertrules', 'monitoring/silences'];
const clusterSettingsStartsWith = ['settings/cluster', 'settings/idp', 'config.openshift.io', 'monitoring/alertmanagerconfig', 'monitoring/alertmanageryaml'];
...
const MonitoringNavSection_ = ({ canAccess, kibanaURL }) => {
  ...
  return canAccessPrometheus || showSilences || kibanaURL ? (
    <NavSection title="Monitoring">
      {canAccessPrometheus && <HrefLink href="/monitoring/alerts" name="Alerting" startsWith={monitoringAlertsStartsWith} />}
      {canAccessPrometheus && <HrefLink href="/monitoring/query-browser?query0=" name="Metrics" startsWith={['monitoring/query-browser']} />}
    ...
    {t => (
      <>
        ...
        <NavSection title="Service Catalog" required={FLAGS.SERVICE_CATALOG}>
          <HrefLink href="/provisionedservices" name="Provisioned Services" activePath="/provisionedservices/" startsWith={provisionedServicesStartsWith} />
          <HrefLink href="/brokermanagement" name="Broker Management" activePath="/brokermanagement/" startsWith={brokerManagementStartsWith} />
        </NavSection>
        ...
        <NavSection title="Compute" required={FLAGS.CAN_LIST_NODE}>
          ...
          <HrefLink href={formatNamespacedRouteForResource(referenceForModel(MachineModel), 'openshift-machine-api')} name="Machines" required={FLAGS.CLUSTER_API} />
          <HrefLink href={formatNamespacedRouteForResource(referenceForModel(MachineSetModel), 'openshift-machine-api')} name="Machine Sets" required={FLAGS.CLUSTER_API} />
          <HrefLink href={formatNamespacedRouteForResource(referenceForModel(MachineAutoscalerModel), 'openshift-machine-api')} name="Machine Autoscalers" required={FLAGS.MACHINE_AUTOSCALER} />
          <HrefLink href={formatNamespacedRouteForResource(referenceForModel(MachineHealthCheckModel), 'openshift-machine-api')} name="Machine Health Checks" required={FLAGS.MACHINE_HEALTH_CHECK} />
        ...
        <NavSection title="User Management">
          ...
          <ResourceClusterLink resource={referenceForModel(GroupModel)} name="Groups" required={[FLAGS.OPENSHIFT, FLAGS.CAN_LIST_GROUPS]} />

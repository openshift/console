...
import { connectToExtensions, isYAMLTemplate } from '@console/plugin-sdk';
...
const generateObjToLoad = (kind, templateName, namespace, pluginYAMLTemplates) => {
  const sampleObj = safeLoad(getYAMLTemplates(pluginYAMLTemplates).getIn([kind, templateName]));
...
const extensionsToProps = (extensions) => ({
    ...
    class EditYAML extends React.Component {
      ...
      loadSampleYaml_(templateName = 'default', kind = referenceForModel(this.props.model)) {
        const sampleObj = generateObjToLoad(
          kind,
          templateName,
          this.props.obj.metadata.namespace || 'default',
          this.props.pluginYAMLTemplates,
        );
      ...
      downloadSampleYaml_(templateName = 'default', kind = referenceForModel(this.props.model)) {
        const data = safeDump(generateObjToLoad(
          kind,
          templateName,
          this.props.obj.metadata.namespace || 'default',
          this.props.pluginYAMLTemplates,

...
export const podCompare = (pod1: PodKind, pod2: PodKind): number => {
  ...
  return pod1.metadata?.name?.localeCompare(pod2.metadata?.name || '') || 0;
};
  ...
  return (
    <li className="list-group-item container-fluid">
      <div className="row">
        ...
        <span className="col-xs-3 pf-v6-u-text-align-right">
...
export const PodsOverviewContent: React.SFC<PodsOverviewContentProps> = ({
  ...
  const showWaitingForBuildAlert =
    buildConfigData?.buildConfigs?.length &&
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      let updatedPods = podResources?.length
        ? podResources.reduce<PodKind[]>((acc, resource) => {

...
export const AlertRoutingModal: React.FC<AlertRoutingModalProps> = ({config, secret, cancel, close}) => {
  ...
  const submit = (event): void => {
    ...
    _.set(configObj, ['route', 'group_by'], event.target.elements['input-group-by'].value.split(','));
    _.set(configObj, ['route', 'group_wait'], event.target.elements['input-group-wait'].value);
    _.set(configObj, ['route', 'group_interval'], event.target.elements['input-group-interval'].value);
    _.set(configObj, ['route', 'repeat_interval'], event.target.elements['input-repeat-interval'].value);
    ...
    setInProgress(true);
    k8sPatch(SecretModel, secret, [{ op: 'replace', path: '/data/alertmanager.yaml', value:  Base64.encode(yamlStringData)}])
      .then(close, err => {
  ...
  return <form onSubmit={submit} name="form" className="modal-content">
        ...
        <div className="co-m-form-col col-sm-12">
          <div className="form-inline">
            <div className="pf-c-input-group">
              <input placeholder="cluster, alertname"
                type="text"
    ...
    <ModalSubmitFooter inProgress={inProgress} errorMessage={errorMessage} cancel={cancel} submitText="Save" />
  </form>;
};

export const createAlertRoutingModal = createModalLauncher<AlertRoutingModalProps>(AlertRoutingModal);

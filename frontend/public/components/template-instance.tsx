...
export const TemplateInstanceList: React.FC<TemplateInstanceListProps> = ({
  ...
  const templateInstanceStatusFilterOptions = React.useMemo<DataViewFilterOption[]>(() => {
    ...
    const statusCounts = data.reduce((counts, instance) => {
    ...
    return [
      {
        value: 'Ready',
        label: `${t('public~Ready')} (${statusCounts.Ready || 0})`,
      },
      {
        value: 'Not Ready',
        label: `${t('public~Not Ready')} (${statusCounts['Not Ready'] || 0})`,
      },
      {
        value: 'Failed',
  ...
  const additionalFilterNodes = React.useMemo<React.ReactNode[]>(
    ...
    [templateInstanceStatusFilterOptions, t],
  );

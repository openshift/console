...
const MastheadToolbarContents = ({ consoleLinks, cv, isMastheadStacked }) => {
  ...
  const getStatuspageData = (statuspageID) => {
  ...
  const getImportYAMLPath = () => {
    return formatNamespacedRouteForResource('import', activeNamespace);
  };
  ...
  React.useEffect(() => {
    if (window.SERVER_FLAGS.statuspageID) {
      getStatuspageData(window.SERVER_FLAGS.statuspageID);
    }

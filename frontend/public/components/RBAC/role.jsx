...
const menuActions = [
  ...
  (kind, role) => ({
    ...
    href: `/k8s/cluster/rolebindings/_new?rolekind=${roleKind(role)}&rolename=${role.metadata.name}`,
  }),
  Cog.factory.Edit,
  Cog.factory.Delete,
];
...
export const BindingsForRolePage = (props) => {
  ...
  return <MultiListPage
    ...
    createProps={{to: `/k8s/${ns ? `ns/${ns}` : 'cluster'}/rolebindings/_new?rolekind=${kind}&rolename=${name}`}}
...
export const RolesPage = connectToFlags(FLAGS.PROJECTS_AVAILBLE, FLAGS.PROJECTS_AVAILBLE)(({namespace, showTitle, flags}) => {
  ...
  return <MultiListPage
    ...
    createProps={{to: `/k8s/ns/${namespace || 'default'}/roles/_new`}}

...
import { connectToExtensions, Extension, RoutePage, isRoutePage } from '@console/plugin-sdk';
...
const getPageRouteExtensions = (perspective: string, pluginRoutePages: RoutePage[]) =>
  pluginRoutePages.map((r) => {
...
const mapExtensionsToProps = (extensions: Extension[]) => ({
  pluginRoutePages: extensions.filter(isRoutePage),
});

// use `withRouter` to force a re-render when routes change since we are using React.memo
const AppContents = connect(mapStateToProps)(connectToExtensions(mapExtensionsToProps)(withRouter<AppContentsProps>(
  React.memo(({ activePerspective, pluginRoutePages }) =>
    <PageSection variant={PageSectionVariants.light}>
      <div id="content">
        ...
        <div id="content-scrollable">
          <Switch>
            {getPageRouteExtensions(activePerspective, pluginRoutePages)}
...
type AppContentsProps = RouteComponentProps & {
  activePerspective: string;
  pluginRoutePages: RoutePage[];
};

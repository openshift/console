...
describe('<AreaChart />', () => {
  ...
  it('should render an area chart', () => {
    renderWithProviders(
      <AreaChart data={MOCK_DATA} loading={false} height={120} title="Test Area Chart" />,
    );
  ...
  it('should not render any axis', () => {
    renderWithProviders(<AreaChart data={MOCK_DATA} xAxis={false} yAxis={false} loading={false} />);
  ...
  it('should show an empty state', () => {
    renderWithProviders(<AreaChart data={[]} loading={false} />);

    expect(screen.getByText('No datapoints found.')).toBeVisible();
  });

  it('should show a loading state', () => {
    renderWithProviders(<AreaChart data={[[]]} loading={true} title="Loading Chart" />);

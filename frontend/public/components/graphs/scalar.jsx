...
export class Scalar extends SafetyFirst {
  ...
  fetch () {
    this.props.fetch()
      ...
      .then(() => setTimeout(() => this.fetch(), 5000));
  }
  ...
  render () {
    ...
    return <div style={Object.assign({}, {border: '1px solid #ddd', borderRadius: 8, padding: 15, margin: '8px 0', height: 200}, this.style)} >
      { title && <h4 style={{fontWeight: 'bold', margin: 0, textAlign: 'center', color: '#444'}}>{title}</h4> }
      <div className="text-center">
        <h1 style={{color, fontSize: 48, fontWeight: 'bold', padding: '28px 0 0', marginBottom: 0}}>{short}</h1>

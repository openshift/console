...
export const addNIC = (nic: Network) => {
  ...
  cy.get(nicDialog.nicName).clear().type(nic.name);
  if (nic.model) {
    cy.get(nicDialog.model).click();
    cy.get(menuItemMain).contains(nic.model).click();
  }
  cy.get(nicDialog.NAD).select(nic.nad).should('have.value', nic.nad);
  if (nic.type) {
    cy.get(nicDialog.nicType).click();
    cy.get(menuItemMain).contains(nic.type).click();
  }
...
export const addDisk = (disk: Disk) => {
  ...
  if (disk.source) {
    ...
    cy.get(menuItemMain).contains(disk.source.getDescription()).click();
  ...
  cy.get(diskDialog.diskName).clear().type(disk.name);
  if (disk.size) {
    cy.get(diskDialog.size).type(disk.size);
  }
  if (disk.drive) {
    cy.get(diskDialog.diskType).select(disk.drive).should('have.value', disk.drive);
  }
  if (disk.interface) {
    cy.get(diskDialog.diskInterface).click();
    cy.get(menuItemMain).contains(disk.interface).click();
  }
  ...
  if (disk.autoDetach === true) {
    cy.get(diskDialog.autoDetach).check().should('be.checked');
  } else if (disk.autoDetach === false) {
    cy.get(diskDialog.autoDetach).uncheck().should('not.be.checked');
  }
...
export const deleteRow = (rowID: string) => {
  ...
  cy.byDataID(rowID).find('[data-test-id="kebab-button"]').click();

...
describe('Test VM dedicated resources', () => {
  ...
  it('ID(CNV-3731) Enable dedicated resources guaranteed policy', () => {
    cy.get(dedicatedResources).find(detailsTab.vmEditWithPencil).click();
    ...
    cy.exec(getDedicatedCPU).its('stdout').should('equal', 'true');
    // uncheck it again;
    cy.get(dedicatedResources).find(detailsTab.vmEditWithPencil).click();
  ...
  it('ID(CNV-7245) Enable/disable VM eviction strategy', () => {
    if (Cypress.env('DOWNSTREAM')) {
      ...
      cy.get(evictionStrategy).find(detailsTab.vmEditWithPencil).click();
  ...
  it('ID(CNV-3076) Change VM flavor', () => {
    ...
    cy.get(flavor).find(detailsTab.vmEditWithPencil).click();

...
describe('Test VM details tab', () => {
  ...
  it('ID(CNV-3550) Displays boot devices', () => {
    cy.get(detailsTab.vmBootOrder).find('li').first().should('contain', BootDevices.containerDisk);
  });

  it('ID(CNV-3548) Adds bootable device', () => {
    cy.get('[data-test="boot-order-details-item"]').find(detailsTab.vmEditWithPencil).click();
    ...
    cy.get(detailsTab.vmBootOrder).find('li').eq(0).should('contain', BootDevices.containerDisk);
    cy.get(detailsTab.vmBootOrder).find('li').eq(1).should('contain', BootDevices.defaultNIC);
  });
  ...
  it('ID(CNV-3547) Drags and drops to change boot order', () => {
    ...
    cy.get(draggablePointer).eq(0).trigger('dragstart', { dataTransfer });

    cy.get(draggablePointer).eq(1).trigger('drop').trigger('dragend', { dataTransfer });

    cy.get(modalConfirm).click();
    cy.get(modalTitle).should('not.exist');
    cy.reload(); // ensure page is refreshed
    cy.contains(BootDevices.defaultNIC).should('exist');
    cy.get(detailsTab.vmBootOrder).find('li').eq(0).should('contain', BootDevices.defaultNIC);
  });

  it('ID(CNV-3549) Deletes bootable device', () => {
    cy.get('[data-test="boot-order-details-item"]').find(detailsTab.vmEditWithPencil).click();
    cy.get(deletePointer).eq(0).click(); // delete nic
    cy.get(modalConfirm).click();
    cy.get(modalTitle).should('not.exist');
    cy.reload(); // ensure page is refreshed

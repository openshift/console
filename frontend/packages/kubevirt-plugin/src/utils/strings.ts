...
export const UI_SOURCE_CONTAINER_DESC = `Upload content from a container located in a registry accessible from the cluster. Example: ${EXAMPLE_CONTAINER}. A new persistent volume claim (PVC) will be created`;
...
export const PROVISION_SOURCE_CONTAINER_DESC = `Link to a bootable operating system container located in a registry accessible from the cluster. Example: ${EXAMPLE_CONTAINER}. This will show up as a disk in the Storage step`;
...
export const NIC_MODEL_VIRTIO_DESC =
  'Optimized for best performance. Windows VMs require virtio drivers to function.';

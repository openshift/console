...
const getConsolePluginAPIs = () => {
  ...
  return typeChecker
    ...
    .reduce<PluginAPIInfo[]>((acc, symbol) => {
      ...
      const sdkPath = relativePath(declaration.getSourceFile().fileName);
      ...
      if (jsDocs.length === 0) {
        acc.push({
          ...
          sdkPath,
          doc: {
            summary: `Documentation not available, please refer to the implementation.`,
          },
        });
...
renderTemplate('scripts/templates/api.md.ejs', {
  apis: getConsolePluginAPIs()
    // Sort APIs by name, list non-deprecated and non-type APIs first
    .sort((a, b) => {

 ...
 * ```
 * @param typeGuards A list of callbacks that each accept a dynamic plugin extension as an argument
 * and return a boolean flag indicating whether or not the extension meets desired type constraints
 *
...
/**
  ...
  @returns Tuple `[activeColumns, userSettingsAreLoaded]` where:
    `activeColumns` is a user selected subset of options.columns parameter
    `userSettingsAreLoaded` is a boolean flag indicating whether user settings have been loaded.
  @example
  ```tsx
...
/**
  ...
  @param rowFilters - (optional) array of RowFilter elements that define the available filter options
  @param staticFilters - (optional) array of FilterValue elements that are statically applied to the data
  @returns Tuple `[staticFilteredData, filteredData, onFilterChange]` where:
    `staticFilteredData` is the result of applying staticFilters to data
    `filteredData` is the result of applying rowFilters to staticFilteredData
    `onFilterChange` is a callback that allows calling component to update rowFilters
  @example
  ```tsx

    ...
    <>
      <div className={cx('ocs-synced-editor-field__editor-toggle', { margin: !noMargin })}>
        <RadioGroupField
          ...
          options={[
            {
              label: formContext.label || t('console-dynamic-plugin-sdk~Form view'),
              value: EditorType.Form,
              isDisabled: formContext.isDisabled,
            },
            {
              label: yamlContext.label || t('console-dynamic-plugin-sdk~YAML view'),
      ...
      {yamlWarning && (
        <Alert
          ...
          variant="danger"
          isInline
          title={t('console-dynamic-plugin-sdk~Invalid YAML cannot be persisted')}
        >
          <p>
            {t('console-dynamic-plugin-sdk~Switching to form view will delete any invalid YAML.')}
          </p>
          <Button variant="danger" onClick={onClickYAMLWarningConfirm}>
            {t('console-dynamic-plugin-sdk~Switch and delete')}
          </Button>
          &nbsp;
          <Button variant="secondary" onClick={onClickYAMLWarningCancel}>
            {t('console-dynamic-plugin-sdk~Cancel')}
          </Button>
        </Alert>
      )}
      {disabledFormAlert && (
        <Alert
          variant="default"
          title={t(
            'console-dynamic-plugin-sdk~Form view is disabled for this chart because the schema is not available',
          )}

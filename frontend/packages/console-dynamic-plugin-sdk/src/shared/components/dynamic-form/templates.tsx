...
const AdvancedProperties: React.FC<Pick<ObjectFieldTemplateProps, 'properties'>> = ({
  ...
  return (
    <ExpandCollapse
      textCollapsed={t('console-dynamic-plugin-sdk~Advanced configuration')}
      textExpanded={t('console-dynamic-plugin-sdk~Advanced configuration')}
    >
    ...
    >
            ...
            {item.hasRemove && (
              <div className="row co-dynamic-form__array-field-group-remove">
                ...
                >
                  ...
                  {t('console-dynamic-plugin-sdk~Remove {{singularLabel}}', { singularLabel })}
                </Button>
              </div>
            )}
            {item.children}
      ...
      <div className="row">
        <Button id={`${idSchema.$id}_add-btn`} type="button" onClick={onAddClick} variant="link">
          ...
          {t('console-dynamic-plugin-sdk~Add {{singularLabel}}', { singularLabel })}
        </Button>
      </div>
    </FieldSet>
  );
...
export const ErrorTemplate: React.FC<{ errors: string[] }> = ({ errors }) => {
  ...
  return (
    <Alert
      ...
      variant="danger"
      title={t('console-dynamic-plugin-sdk~Error')}
    >
      {t('console-dynamic-plugin-sdk~Fix the following errors:')}
      <ul>
        {_.map(errors, (error) => (

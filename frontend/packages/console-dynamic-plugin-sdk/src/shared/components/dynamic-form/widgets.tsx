...
export const SwitchWidget: React.FC<WidgetProps> = ({ value, id, label, onChange, options }) => {
  ...
  const {
    labelOn = t('console-dynamic-plugin-sdk~true'),
    labelOff = t('console-dynamic-plugin-sdk~false'),
  } = options;
  return (
    <Switch
      id={id || label}
    ...
    <div>
      {!_.isUndefined(model) ? (
        <ListDropdown
          ...
          desc={label}
          placeholder={t('console-dynamic-plugin-sdk~Select {{label}}', { label: model.label })}
          onChange={(next) => onChange(next)}
          selectedKey={selectedKey}
        />
      ) : (
        <span>
          {t('console-dynamic-plugin-sdk~Cluster does not have resource {{groupVersionKind}}', {
            groupVersionKind,
          })}
        </span>
      )}
...
export const SelectWidget: React.FC<WidgetProps> = ({
  ...
  return (
    <Dropdown
      id={id}
      key={id}
      title={t('console-dynamic-plugin-sdk~Select {{title}}', {
        title: title || schema?.title || label,
      })}
      selectedKey={value}
      items={items}

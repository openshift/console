...
const ResourceQuotaBody: React.FC<ResourceQuotaBodyProps> = ({ error, isLoading, children }) => {
  ...
  if (error) {
    body = <div className="text-secondary">{t('console-dynamic-plugin-sdk~Not available')}</div>;
  } else if (isLoading) {
    body = <div className="skeleton-quota" />;
  } else if (!React.Children.count(children)) {
    body = (
      <div className="text-secondary">{t('console-dynamic-plugin-sdk~No resource quotas')}</div>
    );
  }

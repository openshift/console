    ...
    <div className="co-activity-item__ongoing" data-test="activity">
      {timestamp && (
        <span className="text-secondary">
          {t('console-dynamic-plugin-sdk~Started')}{' '}
    ...
    return (
      <Activity>
        <div className="text-secondary">
          {t('console-dynamic-plugin-sdk~There are no recent events.')}
        </div>
      </Activity>
    );
  }
  return (
    <>
      ...
      {sortedEvents.length > 50 && !!moreLink && (
        ...
        >
          {t('console-dynamic-plugin-sdk~View all events')}
        </Link>
      )}
    </>
  );
...
export const PauseButton: React.FC<PauseButtonProps> = ({ paused, togglePause }) => {
  ...
  return (
    ...
    >
      {paused ? t('console-dynamic-plugin-sdk~Resume') : t('console-dynamic-plugin-sdk~Pause')}
    </DashboardCardButtonLink>
  );
};
  ...
  return (
    <>
      <div className="co-activity-card__recent-title" data-test="activity-recent-title">
        {t('console-dynamic-plugin-sdk~Recent events')}
...
export const OngoingActivityBody: React.FC<OngoingActivityBodyProps> = ({
  ...
  if (!activitiesLoaded) {
    ...
    body = allActivities.length ? (
      ...
      <Activity>
        <div className="text-secondary">
          {t('console-dynamic-plugin-sdk~There are no ongoing activities.')}
        </div>
      </Activity>
    );
  }
  return (
    <>
      <div className="co-activity-card__ongoing-title" data-test="ongoing-title">
        {t('console-dynamic-plugin-sdk~Ongoing')}
      </div>

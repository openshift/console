  ...
  ({ current, title, humanize, consumers, namespace, position, description, children }) => {
    ...
    return (
      <DashboardCardPopupLink
        popupTitle={t('console-dynamic-plugin-sdk~{{title}} breakdown', { title })}
        linkTitle={current}
  ...
  return (
    ((!!limitState && limitState !== LIMIT_STATE.OK) ||
      ...
      <ul className="co-utilization-card-popover__consumer-list">
        <Status value={total}>{t('console-dynamic-plugin-sdk~Total capacity')}</Status>
        <Status value={limit} icon={getLimitIcon(limitState)}>
          {t('console-dynamic-plugin-sdk~Total limit')}
        </Status>
        <Status value={current}>{t('console-dynamic-plugin-sdk~Current utilization')}</Status>
        <Status value={available}>
          {t('console-dynamic-plugin-sdk~Current available capacity')}
        </Status>
        <Status value={requested} icon={getLimitIcon(requestedState)}>
          {t('console-dynamic-plugin-sdk~Total requested')}
        </Status>
      </ul>
    )
  );
  ...
  React.memo(
    ({
      ...
      const dropdownItems = React.useMemo(
        () =>
          consumers.reduce((items, curr) => {
            items[referenceForModel(curr.model)] = t('console-dynamic-plugin-sdk~By {{label}}', {
      ...
      if (error || consumersLoadError) {
        body = (
          <div className="text-secondary">{t('console-dynamic-plugin-sdk~Not available')}</div>
        );
        ...
        body = (
          <>
            <ul
              ...
              aria-label={t('console-dynamic-plugin-sdk~Top consumer by {{label}}', {
                label: model.label,
              })}
            >
              {top5Data &&
            ...
            <Link to={monitoringURL}>{t('console-dynamic-plugin-sdk~View more')}</Link>
          </>
        );
      }
      ...
      return (
        <div className="co-utilization-card-popover__body">
          ...
          <div className="co-utilization-card-popover__title">
            {consumers.length === 1
              ? t('console-dynamic-plugin-sdk~Top {{label}} consumers', {
                  label: currentConsumer.model.label.toLowerCase(),
                })
              : t('console-dynamic-plugin-sdk~Top consumers')}
          </div>
          {consumers.length > 1 && (
            <Dropdown
              ...
              aria-label={t('console-dynamic-plugin-sdk~Select consumer type')}
              items={dropdownItems}

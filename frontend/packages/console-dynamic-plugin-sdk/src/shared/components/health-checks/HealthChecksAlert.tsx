...
const HealthChecksAlert: React.FC<HealthChecksAlertProps> = ({ resource }) => {
  ...
  const alertMessage =
    _.size(containersName) > 1
      ? t(
          'console-dynamic-plugin-sdk~Not all Containers have health checks to ensure your Application is running correctly.',
        )
      : t(
          'console-dynamic-plugin-sdk~Container {{containersName}} does not have health checks to ensure your Application is running correctly.',
    ...
    <>
      {showAlert ? (
        <div className="ocs-health-checks-alert">
          <Alert
            variant="default"
            title={t('console-dynamic-plugin-sdk~Health checks')}
            actionClose={<AlertActionCloseButton onClose={handleAlertAction} />}
            isInline
          >
            {alertMessage}{' '}
            <Link to={addHealthChecksLink}>
              {t('console-dynamic-plugin-sdk~Add health checks')}
            </Link>
          </Alert>
        </div>
      ) : null}

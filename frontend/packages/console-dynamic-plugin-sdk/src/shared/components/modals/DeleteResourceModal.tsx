...
const DeleteResourceForm: React.FC<FormikProps<FormikValues> & DeleteResourceModalProps> = ({
  ...
  resourceName,
  resourceType,
  // t('console-dynamic-plugin-sdk~Delete')
  actionLabelKey = 'console-dynamic-plugin-sdk~Delete',
  isSubmitting,
  cancel,
  values,
  status,
  ...
  return (
    <form onSubmit={handleSubmit} className="modal-content modal-content--no-inner-scroll">
      ...
      <ModalBody>
        <p>
          {t(
            `console-dynamic-plugin-sdk~This action cannot be undone. All associated Deployments, Routes, Builds, Pipelines, Storage/PVCs, Secrets, and ConfigMaps will be deleted.`,
          )}
        </p>
        <p>
          <Trans ns="console-dynamic-plugin-sdk">

  ...
  if (onHide) {
    actionDropdownItem.push(
      <DropdownItem
        ...
        component="button"
        description={t(
          'console-dynamic-plugin-sdk~You can always bring these getting started resources back into view by clicking Show getting started resources in the page heading.',
        )}
        onClick={onHide}
        data-test="hide"
      >
        {t('console-dynamic-plugin-sdk~Hide from view')}
      </DropdownItem>,
    );
  }

  const title = t('console-dynamic-plugin-sdk~Getting started resources');
  const titleTooltip = (
    <span className="ocs-getting-started-grid__tooltip">
      {t(
        'console-dynamic-plugin-sdk~Use our collection of resources to help you get started with the Console.\n\nNote: This card can be hidden at any time.',
      )}
    </span>
  );
        ...
        <CardTitle>
          <Title headingLevel="h2" size={TitleSizes.lg} data-test="title">
            ...
            <Popover bodyContent={titleTooltip}>
              <span
                role="button"
                aria-label={t('console-dynamic-plugin-sdk~More info')}

...
const podKindString = (count: number) =>
  count === 1
    ? i18next.t('console-dynamic-plugin-sdk~Pod')
    : i18next.t('console-dynamic-plugin-sdk~Pods');
...
const getTitleAndSubtitle = (
  ...
  if (!currentPodCount) {
    titlePhrase = isPending ? '0' : t('console-dynamic-plugin-sdk~Scaled to 0');
    longTitle = !isPending;
    if (desiredPodCount) {
      subTitlePhrase = t('console-dynamic-plugin-sdk~Scaling to {{podSubTitle}}', {
  ...
  switch (ownerKind) {
    ...
    case RevisionModel.kind:
      ...
      if (!isPending && !desiredPodCount) {
        podRingLabelData.title = t('console-dynamic-plugin-sdk~Autoscaled');
        podRingLabelData.subTitle = t('console-dynamic-plugin-sdk~to 0');
        podRingLabelData.reversed = true;
        break;
      }
      if (isPending) {
        podRingLabelData.title = '0';
        podRingLabelData.subTitle = t('console-dynamic-plugin-sdk~Scaling to {{podSubTitle}}', {
          podSubTitle: desiredPodCount,
        });
      } else {
...
export const hpaPodRingLabel = (
  ...
  return {
    title: scaling
      ? t('console-dynamic-plugin-sdk~Autoscaling')
      : t('console-dynamic-plugin-sdk~Autoscaled'),
    subTitle: t('console-dynamic-plugin-sdk~to {{count}} Pod', { count: desiredPods }),
    longTitle: false,
    longSubtitle: false,
    reversed: true,
  };

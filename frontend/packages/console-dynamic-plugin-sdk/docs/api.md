...
const Component: React.FC = (props) => {
  ...
  return (
    <select value={activePerspective} onChange={(e) => setActivePerspective(e.target.value)}>
      {
        // ...perspective options
      }
    </select>
...
```tsx
<StatusPopupSection firstColumn="Example" secondColumn="Status">
  <StatusPopupItem icon={healthStateMapping[MCGMetrics.state]?.icon}>Complete</StatusPopupItem>
  <StatusPopupItem icon={healthStateMapping[RGWMetrics.state]?.icon}>Pending</StatusPopupItem>
</StatusPopupSection>
```
...
<React.Suspense fallback={<LoadingBox />}>
  <CodeEditor value={code} language="yaml" />
</React.Suspense>
```
...
const AppPage: React.FC = () => {
  ...
  return <Button onClick={onClick}>Launch a Modal</Button>;
};
```
...
```tsx
const Component: React.FC = (props) => {
  const [state, setState, loaded] = useUserSettings('devconsole.addPage.showDetails', true, true);
  return loaded ? (

...
| Name | Value Type | Optional | Description |
| ---- | ---------- | -------- | ----------- |
| `catalogId` | `string` | no | The catalog ID the alert is for. |
| `component` | `CodeRef<React.ComponentType<{}>>` | no | The component to render as the alert. |
| `type` | `string` | yes | The catalog item type for this alert. If not specified, the alert will be displayed for all types. |
| `priority` | `number` | yes | Priority for this alert. Higher priority alerts are displayed first. Defaults to 0. |

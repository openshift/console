...
- Upgraded from `react-router` v5 to v6. Plugins must use `react-router` v6 to remain compatible with Console.
- The `react-router-dom-v5-compat` shared module is deprecated, and has been aliased to `react-router-dom`.
  for the time being. Plugins targeting 4.22 must update remove their `react-router-dom-v5-compat` dependency
  and use `react-router-dom` instead.

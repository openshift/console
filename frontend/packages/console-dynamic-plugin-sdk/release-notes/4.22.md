...
- The `react-router-dom-v5-compat` and `react-router-dom` shared modules are deprecated and alias to `react-router`
  for backward compatibility. Plugins must replace these imports with `react-router`.
...
Console 4.22 uses `react-router` v7, and as a result, the `react-router-dom-v5-compat` package is deprecated. However,
the `react-router-dom-v5-compat` shared module is an alias to `react-router` so that plugins can continue to use it without
disruption. Except for `CompatRouter`, there are no significant API differences between `react-router-dom-v5-compat` and
`react-router` v7.

In React Router v7, the [`react-router-dom` package merges into `react-router`][react-router-dom collapse]. Console aliases
`react-router-dom` to `react-router` to avoid breaking plugins.

Replace all imports of `react-router-dom-v5-compat` and `react-router-dom` with `react-router` to remain compatible with

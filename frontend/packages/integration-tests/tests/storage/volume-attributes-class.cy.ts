...
describe('VolumeAttributesClass E2E tests', () => {
  ...
  before(() => {
    ...
    // Create hyperdisk-balanced StorageClass for PVC provisioning
    cy.exec(`echo '${JSON.stringify(STORAGE_CLASS)}' | oc apply -f -`);
    // Create VolumeAttributesClasses for testing
    cy.exec(`echo '${JSON.stringify(VAC_STANDARD)}' | oc apply -f -`);
    cy.exec(`echo '${JSON.stringify(VAC_PERFORMANCE)}' | oc apply -f -`);
    cy.exec(`echo '${JSON.stringify(VAC_INVALID)}' | oc apply -f -`);
    // Create Deployment to consume the PVC
  ...
  after(() => {
    ...
    [TEST_VAC_STANDARD, TEST_VAC_PERFORMANCE, TEST_VAC_INVALID].forEach((vacName) => {
      cy.exec(
    ...
    cy.exec(
      `oc delete volumeattributesclass ${TEST_VAC_STANDARD} ${TEST_VAC_PERFORMANCE} ${TEST_VAC_INVALID} --ignore-not-found=true --wait=false`,
      {
        failOnNonZeroExit: false,
        timeout: 30000,
      },
  ...
  it('creates PVC with VolumeAttributesClass and verifies it appears on details page', () => {
    ...
    cy.byTestID('pvc-size').clear().type('10');

    // Select hyperdisk StorageClass from dropdown
    ...
    cy.byTestID('console-select-item').contains(TEST_VAC_STANDARD).click();

    // Create PVC and navigate to details page
    cy.byTestID('create-pvc').click();
    detailsPage.titleShouldContain(TEST_PVC);

    // Verify requested VAC is displayed (spec.volumeAttributesClassName)
    ...
    // Verify current VAC matches requested VAC (status.currentVolumeAttributesClassName)
    cy.byLegacyTestID('pvc-current-vac', { timeout: 120000 }).should('exist');
    cy.byLegacyTestID('pvc-current-vac').should('contain.text', TEST_VAC_STANDARD);
  });
  ...
  it('modifies VolumeAttributesClass via modal and verifies update', () => {
    ...
    // Ensure PVC is bound and current VAC is set before modifying
    resourceStatusShouldContain('Bound', { timeout: 30000 });
    cy.byLegacyTestID('pvc-current-vac', { timeout: 60000 }).should('exist');

    // Open Modify VolumeAttributesClass modal
    detailsPage.clickPageActionFromDropdown('Modify VolumeAttributesClass');
    modal.shouldBeOpened();

    // Select performance tier VolumeAttributesClass
    cy.byTestID('modify-vac-dropdown').click();
    cy.byTestID('console-select-item').contains(TEST_VAC_PERFORMANCE).click();
    modal.submit();
    modal.shouldBeClosed();

    // Verify requested VAC updated to performance tier
    cy.byLegacyTestID('pvc-requested-vac', { timeout: 30000 }).should(
      'contain.text',
      TEST_VAC_PERFORMANCE,
    );

    // Verify current VAC updated to performance tier
    // Uses identical parameters to standard tier, so CSI driver applies change quickly
    cy.byLegacyTestID('pvc-current-vac', { timeout: 120000 }).should(
      'contain.text',
      TEST_VAC_PERFORMANCE,
    );
  });
  ...
  it('attempts invalid VAC modification and verifies error alert', () => {
    ...
    // Ensure PVC is in expected state before modifying
    resourceStatusShouldContain('Bound', { timeout: 30000 });
    ...
    cy.byLegacyTestID('vac-error-alert', { timeout: 120000 }).should('be.visible');
    cy.byLegacyTestID('vac-error-alert').should(
      'contain.text',
      'VolumeAttributesClass modification failed',
    );

    // Current VAC should remain at previous valid value (performance tier)
    cy.byLegacyTestID('pvc-current-vac').should('contain.text', TEST_VAC_PERFORMANCE);
  });
});

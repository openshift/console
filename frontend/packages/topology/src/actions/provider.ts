...
import { useDeleteConnectorAction, MoveConnectorAction } from './edgeActions';
...
export const useTopologyVisualConnectorActionProvider = (element: Edge) => {
  ...
  const actions = useMemo(() => {
    ...
    return [MoveConnectorAction(kindObj, element), deleteConnectorAction];
  }, [element, kindObj, deleteConnectorAction]);

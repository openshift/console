...
import { switchPerspective, Perspective, sideHeader } from '../views/hc-perspective.view';
import {
  pipelinecheckStatus,
  pageSidebar,
  pipelinePage,
...
describe('Pipeline', async () => {
  ...
  it('pipeline is installed', async () => {
    ...
    await switchPerspective(Perspective.HyperCloud);
    expect(sideHeader.getText()).toContain('HyperCloud');
  ...
  it('create pipeline through cli & UI', async () => {
    ...
    await execSync(
      `oc create -f ./packages/hc-console/integration-tests/views/simple-pipeline-demo.yaml -n ${testName}`,
    );
    await pipelinecheckStatus();
  ...
  it('run pipeline and generate pipeline logs scenario', async () => {
    ...
    await pipelinecheckStatus();
    await execSync(
      `oc create -f ./packages/hc-console/integration-tests/views/sample-app-pipeline.yaml -n ${testName}`,
    );

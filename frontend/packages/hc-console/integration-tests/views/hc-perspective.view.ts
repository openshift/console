...
export const hcPerspective = element(
  by.cssContainingText('.pf-c-dropdown__menu-item', 'HyperCloud'),
);

export enum Perspective {
  Developer = 'Developer Perspective',
  Administrator = ' Administrator Perspective',
  HyperCloud = 'HyperCloud Perspective',
}
...
export const switchPerspective = async function(perspective: Perspective) {
  ...
  switch (perspective) {
    ...
    case Perspective.HyperCloud: {
      await browser.wait(until.elementToBeClickable(hcPerspective));
      await hcPerspective.click();
      break;
    }

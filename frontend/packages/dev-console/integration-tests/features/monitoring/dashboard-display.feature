...
Feature: Monitoring Page
    ...
    Background:
        ...
        And user has selected existing namespace or created namespace "aut-monitoring"
    ...
    Scenario: Dashboard tab on the Monitoring page for particular workload: MH-01-TC03
        Given user has created workload "national-parks-test" with resource type "Deployment"
        ...
        And user will see the Rate of Transmitted Packets
        And user will see the Rate of Received Packets Dropped
        And user will see the Rate of Transmitted Packets Dropped


    @regression
    Scenario: Dashboard tab on the Monitoring page for all workloads: MH-01-TC02
        Given user has created workload "national-parks-test-1" with resource type "Deployment"
    ...
    @smoke
    Scenario: Alerts details on monitoring page: MH-01-TC04
        Given user is on Monitoring page

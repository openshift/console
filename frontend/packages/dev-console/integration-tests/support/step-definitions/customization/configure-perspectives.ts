...
When('user searches {string}', (value: string) => {
  //   cy.get(search).should('be.visible').type(value);
  //   app.waitForLoad();
  //   cy.get('table[role="grid"]').should('be.visible');
...
// When('user selects {string} console', (version: string) => {
//   cy.get(`[data-test-id="Console"][href*="${version}"]`).should('be.visible').click();

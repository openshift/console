...
export const channelPage = {
  ...
  enterChannelName: (channelName: string) =>
    cy.get(channelPO.channelName).clear().type(channelName),
  enterAppName: (appName: string) => {
    cy.get(channelPO.appName).then(($el) => {
      if ($el.prop('tagName').includes('button')) {
        ...
        cy.get(channelPO.appName).scrollIntoView().invoke('val').should('not.be.empty');
        cy.get(channelPO.appName).clear().type(appName).should('have.value', appName);
      } else {

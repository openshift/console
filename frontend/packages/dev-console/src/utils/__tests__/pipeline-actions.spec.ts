...
describe('getPipelineKebabActions', () => {
  it('expect Remove Trigger option is present', () => {
    ...
    expect(pipelineKebabActions[3](PipelineModel, samplePipeline).label).toBe('Remove Trigger');
  });
  it('expect Remove Trigger option is not present', () => {
    ...
    expect(pipelineKebabActions[3](PipelineModel, samplePipeline).label).not.toBe('Remove Trigger');
  });
  it('expect Start Last Run option is present', () => {
    const pipelineKebabActions = getPipelineKebabActions(samplePipelineRun, false);
    expect(pipelineKebabActions.length).toBe(7);
    expect(pipelineKebabActions[1](PipelineRunModel, samplePipelineRun).label).toBe(
      'Start Last Run',
    );
  });
  it('expect Start Last Run option is not present', () => {
    ...
    expect(pipelineKebabActions[1](PipelineRunModel, samplePipelineRun).label).not.toBe(
      'Start Last Run',
    );
  });

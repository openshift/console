...
test('EnvironmentVariablesSection: should show initial name value pairs', async () => {
  const names = screen.getAllByPlaceholderText(/name/i).map((ele: HTMLInputElement) => ele.value);
  const values = screen.getAllByPlaceholderText(/value/i).map((ele: HTMLInputElement) => ele.value);
  expect(names).toEqual(['xyz', 'xyz2']);
  expect(values).toEqual(['abc', 'abc2']);
});

test('EnvironmentVariablesSection: should add a new row when (+) Add button is clicked', async () => {
...
test('EnvironmentVariablesSection: should add new row with resourse and key dropdowns when (+) Add ConfigMap or Secret button is clicked', async () => {
  const addCMSButton = screen.getByRole('button', { name: /public~add from configmap or secret/i });
...
test('EnvironmentVariablesSection: should remove row when (-) button is clicked', async () => {

...
describe('rerunBuildRun', () => {
  ...
  it('should fail when try to rerun an incomplete BuildRun without build and without buildSpec', async () => {
    const buildRun = incompleteBuildRun;

    await expect(rerunBuildRun(buildRun)).rejects.toEqual(
      new Error('Could not rerun BuildRun without build.name or inline buildSpec.'),
    );

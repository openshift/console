...
import BuildRunDetailsPage from '../BuildRunDetailsPage';
...
(modelFor as jest.Mock).mockReturnValue(BuildRunModel);
(useK8sModel as jest.Mock).mockReturnValue([BuildRunModel]);

describe('BuildRunDetailsPage', () => {
  it('should render a connected SB with the right status and attributes', async () => {
    ...
    const renderResult = render(
      <Wrapper>
        <BuildRunDetailsPage match={matchExistingBuildRun} kind="" />
      </Wrapper>,
    );
    // Consume one rerendering to hide 'test was not wrapped in act' warnings
    await act(async () => null);

    renderResult.getAllByText('BuildRun details');

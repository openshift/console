// Include the cypress customized commands related files
import '@console/cypress-integration-tests/support/selectors';
import '@console/cypress-integration-tests/support/a11y';
import '@console/cypress-integration-tests/support/login';
import '@console/cypress-integration-tests/support/project';
import '@console/cypress-integration-tests/support/index';
import '@dev-console/integration-tests/support/commands/app';
import '@dev-console/integration-tests/support/pageObjects/helm-po';
import './hooks';

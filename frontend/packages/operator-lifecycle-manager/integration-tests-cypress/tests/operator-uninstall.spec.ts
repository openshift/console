...
const navToViewInstancesOfCRD = () => {
  ...
  listPage.filter.byName(testOperand.kind);
  listPage.rows.shouldExist(testOperand.kind);
  listPage.rows.clickRowByName(testOperand.kind);
  detailsPage.isLoaded();
  detailsPage.clickPageActionFromDropdown('View instances');
};
...
const uninstallAndVerify = () => {
  ...
  navToViewInstancesOfCRD();
  listPage.rows.shouldNotExist(testOperand.exampleName);
};
...
describe(`Testing uninstall of ${testOperator.name} Operator`, () => {
  ...
  // TODO this is against Cypress best practices of test independence, but cy.intercepts are only cleared
  // before each test -they are not cleared for after[all]() hook, which is where this should exist

...
import ResourceQuotaCharts from '../ResourceQuotaCharts';

jest.mock('@console/internal/components/graphs/gauge', () => ({
  GaugeChart: jest.fn(({ title, label }: { title: string; label: string }) => `${title} ${label}`),
}));
...
describe('ResourceQuotaCharts', () => {
  ...
  it('should render gauge chart for each resource', () => {

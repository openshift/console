...
const findIndexForItem = (item: NavExtension, currentItems: NavExtension[]): number => {
  ...
  while (count < before.length && index < 0) {
    ...
    index = currentItems.findIndex((i) => i.properties.id === before[countCopy]);
    count++;
  }
  count = 0;
  while (count < after.length && index < 0) {
    const countCopy = count;
    index = currentItems.findIndex((i) => i.properties.id === after[countCopy]);
    if (index >= 0) {
      index += 1;
    }
    count++;
  ...
  const dependencies = (id: string, currentDependencies: string[] = []): string[] => {
    ...
    return dependencyIds.reduce((acc, dependencyId) => {
      if (dependencyId) {
        ...
        return [...acc, dependencyId, ...dependencies(dependencyId, [...acc, dependencyId])];
      }
      return acc;
    }, []);
  };

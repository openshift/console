...
const ConsolePluginsList: React.FC<ConsolePluginsListType> = ({ obj }) => {
  ...
  const [pluginsData, pluginsError] = useURLPoll<ConsolePluginsListResponseType>(
    `${window.SERVER_FLAGS.basePath}api/plugins`,
  );

  const prevPluginsDataRef = React.useRef<ConsolePluginsListResponseType>();
  React.useEffect(() => {
  ...
  const pluginsChanged = !_.isEqual(prevPluginsData?.plugins, pluginsData?.plugins);
  const consoleCommitChanged = !_.isEqual(
    prevPluginsData?.consoleCommit,
    pluginsData?.consoleCommit,
  );
...
type ConsolePluginsListResponseType = {

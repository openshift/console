...
import type { RemediationTemplateRef } from '../../utils/HighAvailabilityUtils';
import { getRemediationTemplateRefsFromHealthCheck } from '../../utils/HighAvailabilityUtils';
  ...
  return (
    <>
      <Title headingLevel="h3" className="co-section-heading">
        <span>{t('Node remediation agents')}</span>
      </Title>
      {isLoading ? (
        <div className="loading-skeleton--table pf-v6-u-w-100" />
      ) : loadError ? (
        t('Unable to load remediation agents')
      ) : (
          ...
          <table className="pf-v6-c-table pf-m-compact pf-m-border-rows">
            <thead className="pf-v6-c-table__thead">
              <tr className="pf-v6-c-table__tr">
                ...
                <th className="pf-v6-c-table__th">{t('Triggered by')}</th>
                <th className="pf-v6-c-table__th">{t('Config object')}</th>
                <th className="pf-v6-c-table__th">{t('Last action')}</th>
              </tr>
            </thead>
              ...
              {rows.length === 0 ? (
                <tr className="pf-v6-c-table__tr">
                  <td className="pf-v6-c-table__td" colSpan={4}>
                    {t('No matching remediation agents found')}
                  </td>
                </tr>
              ) : (
                rows.map((row) => (

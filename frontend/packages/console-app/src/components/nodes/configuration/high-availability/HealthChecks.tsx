  ...
  return (
    <>
      <Title headingLevel="h3" className="co-section-heading">
        <span>{t('Machine/Node health checks')}</span>
      </Title>
      {isLoading ? (
        <div className="loading-skeleton--table pf-v6-u-w-100" />
      ) : loadError ? (
        t('Unable to load health checks')
      ) : (
          ...
          <table className="pf-v6-c-table pf-m-compact pf-m-border-rows">
            <thead className="pf-v6-c-table__thead">
              <tr className="pf-v6-c-table__tr">
                ...
                <th className="pf-v6-c-table__th">{t('Scope')}</th>
                <th className="pf-v6-c-table__th">{t('Selector')}</th>
                <th className="pf-v6-c-table__th">{t('Unhealthy conditions')}</th>
                <th className="pf-v6-c-table__th">{t('Last triggered')}</th>
              </tr>
            </thead>
              ...
              {rows.length === 0 ? (
                <tr className="pf-v6-c-table__tr">
                  <td className="pf-v6-c-table__td" colSpan={5}>
                    {t('No matching MachineHealthChecks or NodeHealthChecks')}
                  </td>
                </tr>
              ) : (
                rows.map((row) => (

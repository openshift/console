...
describe('BMCConfiguration', () => {
  ...
  it('should render an info alert when the bare metal plugin is not active', () => {
    ...
    expect(screen.getByText('Bare Metal is not configured for this cluster.')).toBeInTheDocument();
  });
  ...
  it('should show loading skeletons while BMC data is loading', () => {
    ...
    expect(screen.getByRole('heading', { name: 'BMC Configuration' })).toBeInTheDocument();
    const skeletons = screen.getAllByTestId('config-property-skeleton');
  ...
  it('should display a message when no bare metal host is associated with the node', () => {
    ...
    expect(
      screen.getByText('This node does not have an associated BMC configuration.'),
    ).toBeInTheDocument();
  });

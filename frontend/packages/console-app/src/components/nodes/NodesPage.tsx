...
const NodeList: React.FC<NodeListProps> = ({
  ...
  const nodeStatusFilterOptions = React.useMemo<DataViewFilterOption[]>(() => {
    ...
    const statusCounts = data.reduce((counts, node) => {
      const isCSR = isCSRResource(node);
      const status = isCSR ? 'Discovered' : nodeStatus(node as NodeKind);
    ...
    return [
      {
        value: 'Ready',
        label: `${t('console-app~Ready')} (${statusCounts.Ready || 0})`,
      },
      {
        value: 'Not Ready',
        label: `${t('console-app~Not Ready')} (${statusCounts['Not Ready'] || 0})`,
      },
      {
        value: 'Discovered',
        label: `${t('console-app~Discovered')} (${statusCounts.Discovered || 0})`,
      },
    ];
  }, [t, data]);
  ...
  const nodeRoleFilterOptions = React.useMemo<DataViewFilterOption[]>(() => {
    ...
    const roleCounts = data.reduce((counts, node) => {
      if (!isCSRResource(node)) {
        const roles = getNodeRoles(node as NodeKind);
        roles.forEach((role) => {
    ...
    return [
      {
        value: 'control-plane',
        label: `${t('console-app~control-plane')} (${roleCounts['control-plane'] || 0})`,
      },
      {
        value: 'worker',
        label: `${t('console-app~worker')} (${roleCounts.worker || 0})`,
      },
    ];
  }, [t, data]);

  const nodeArchitectureFilterOptions = React.useMemo<DataViewFilterOption[]>(() => {
    // Calculate node counts by architecture
    const archCounts = data.reduce((counts, node) => {
      if (!isCSRResource(node)) {
        const arch = getNodeArchitecture(node as NodeKind);
        if (arch) {
    ...
    return [
      { value: 'amd64', label: `amd64 (${archCounts.amd64 || 0})` },
      { value: 'ppc64le', label: `ppc64le (${archCounts.ppc64le || 0})` },
      { value: 'arm64', label: `arm64 (${archCounts.arm64 || 0})` },
      { value: 's390x', label: `s390x (${archCounts.s390x || 0})` },
    ];
  }, [data]);
  ...
  const additionalFilterNodes = React.useMemo<React.ReactNode[]>(
    ...
    [nodeStatusFilterOptions, nodeRoleFilterOptions, nodeArchitectureFilterOptions, t],
  );

...
import { MachineKind, MachineHealthCheckKind, NodeKind } from '@console/internal/module/k8s';
...
const LimitLink: React.FC<LimitLinkProps> = ({
  ...
  const nodeIP = getNodeAddresses(obj || ({} as NodeKind)).find((addr) => addr.type === 'InternalIP')?.address;
...
const HealthChecksLink: React.FC = () => {
  ...
  const healthState = getMachineHealth(obj || ({} as NodeKind), machine, healthChecks);
  const { t } = useTranslation();
  return (
    <PFPopover

...
describe('NodeGroupUtils', () => {
  describe('getGroupsFromGroupLabel', () => {
    it('should split comma-separated groups', () => {
      const result = getGroupsFromGroupLabel('group1,group2,group3');
    ...
    it('should return empty array for undefined input', () => {
      const result = getGroupsFromGroupLabel(undefined);
      expect(result).toEqual([]);
    });

    it('should return empty array for empty string', () => {
      const result = getGroupsFromGroupLabel('');
      expect(result).toEqual([]);
    });
    ...
    it('should handle single group', () => {
      const result = getGroupsFromGroupLabel('single-group');
      expect(result).toEqual(['single-group']);
    });

    it('should handle groups with special characters', () => {
      const result = getGroupsFromGroupLabel('prod-us-east-1,dev-eu-west-2');
      expect(result).toEqual(['prod-us-east-1', 'dev-eu-west-2']);
    });
  });

  describe('getNodeGroupLabelFromGroups', () => {
    it('should join groups with comma separator', () => {
      const result = getNodeGroupLabelFromGroups(['group1', 'group2', 'group3']);
      expect(result).toBe('group1,group2,group3');
    });

    it('should handle single group', () => {
      const result = getNodeGroupLabelFromGroups(['single-group']);
      expect(result).toBe('single-group');
    });

    it('should handle empty array', () => {
      const result = getNodeGroupLabelFromGroups([]);
      expect(result).toBe('');
    });

    it('should preserve group order', () => {
      const result = getNodeGroupLabelFromGroups(['z-group', 'a-group', 'm-group']);
  ...
  describe('getNodeGroupLabelFromGroupNameMap', () => {
    it('should return comma-separated groups for a node', () => {
      ...
      const result = getNodeGroupLabelFromGroupNameMap('node1', groupsByName);
    ...
    it('should return undefined when node is not in any group', () => {
      ...
      const result = getNodeGroupLabelFromGroupNameMap('node4', groupsByName);
    ...
    it('should return single group for node in one group', () => {
      ...
      const result = getNodeGroupLabelFromGroupNameMap('node3', groupsByName);
      expect(result).toBe('dev');
    });

    it('should handle empty groupsByName', () => {
      const result = getNodeGroupLabelFromGroupNameMap('node1', {});
      expect(result).toBeUndefined();
    });
  ...
  describe('edge cases and integration', () => {
    it('should handle round-trip: node → groups → label → groups', () => {
      const node: Partial<NodeKind> = {
        metadata: {
          name: 'test-node',
          annotations: {
      ...
      const label = getNodeGroupLabelFromGroups(groups);
      expect(label).toBe('prod,staging,us-east');

      const groupsAgain = getGroupsFromGroupLabel(label);
    ...
    it('should handle complex multi-node scenario', () => {
      ...
      // Get label for specific node
      const node1Label = getNodeGroupLabelFromGroupNameMap('node1', groupsByName);
      expect(node1Label).toBe('prod,us-east');
    });

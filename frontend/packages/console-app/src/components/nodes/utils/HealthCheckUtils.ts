...
import { formatDurationForDisplay } from './HighAvailabilityUtils';

type NodeHealthCheckUnhealthyCondition = {
  type: string;
  status: string;
  duration?: string;
};

type NodeHealthCheckRemediationEntry = {
  started?: string;
  timedOut?: string;
  resource?: {
    name?: string;
...
type NodeHealthCheckUnhealthyNodeEntry = {
  name: string;
...
const CLUSTER_API_MACHINE_SET = 'machine.openshift.io/cluster-api-machineset';
const CLUSTER_API_MACHINE_ROLE = 'machine.openshift.io/cluster-api-machine-role';
const NODE_ROLE_PREFIX = 'node-role.kubernetes.io/';

const nodeHealthChecksWatchResource: WatchK8sResource = {
  groupVersionKind: {
...
const isHealthCheckSelectorEmpty = (selector: Selector | undefined): boolean =>
  new LabelSelector(selector ?? {}).isEmpty();

const getValuesForKey = (reqs: ReturnType<typeof toRequirements>, key: string): string[] =>
  reqs
    .filter(
      (r) =>
        r.key === key &&
...
export const filterMachineHealthChecksForMachine = (
  ...
  // OK to check a new LabelSelector for each health check as there should never be that many

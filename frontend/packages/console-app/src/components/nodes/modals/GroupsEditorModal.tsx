...
const GroupsEditorModal: OverlayComponent<ModalComponentProps> = ({ closeOverlay }) => {
  ...
  const addNewGroup = () => {
    ...
    if (groupNames.length) {
      setGroupsByName((prev) => {
        const newGroupsByName = { ...prev };
        groupNames.forEach((groupName) => {
          if (newGroupsByName[groupName] === undefined) {
            newGroupsByName[groupName] = [];
          }
        });
        return newGroupsByName;
      });
  ...
  const onSubmit = (e): void => {
    ...
    const patchNode = async (
      ...
      const updatedAnnotations = {
        ...
        [GROUP_ANNOTATION]: getNodeGroupAnnotationFromGroups(updatedGroups),
      };
    ...
    const updates = nodes.reduce<Promise<PatchError | undefined>[]>((acc, nextNode) => {
      ...
      const updatedLabel = getNodeGroupAnnotationFromGroupNameMap(
        nextNode.metadata.name,
        groupsByName,
      );
      const updatedGroups = getGroupsFromGroupAnnotation(updatedLabel);

...
const GroupsEditorModal: OverlayComponent<ModalComponentProps> = ({ closeOverlay }) => {
  ...
  useEffect(() => {
    ...
    if (!currentGroupsByName.current) {
      ...
      JSON.stringify(updatedGroupsByName) !== JSON.stringify(currentGroupsByName.current)
    ) {
    ...
    Promise.allSettled(updates)
      .then((results) => {
        ...
        if (failures.length) {
          ...
          patchErrors.splice(0, 0, {
            title: t('console-app~Failed to update {{count}} node(s):', {
      ...
      .catch((error) => {
        setErrors([
          {
            title: t('console-app~Unable to update node(s)'),

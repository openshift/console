...
describe('GroupsEditorModal', () => {
  ...
  describe('Modal rendering', () => {
    ...
    it('shows empty state when no nodes exist', () => {
      ...
      expect(
        screen.getByText('Groups can only be created when there are existing nodes'),
      ).toBeInTheDocument();
    });
  ...
  describe('Adding new groups', () => {
    it('expands Add new group section when clicked', async () => {
      ...
      expect(
        screen.getByPlaceholderText('Enter a group name, then press return, space, or comma'),
      ).toBeInTheDocument();
    });
    ...
    it('adds new group when Enter key is pressed', async () => {
      ...
      const input = screen.getByPlaceholderText(
        'Enter a group name, then press return, space, or comma',
      );
    ...
    it('adds new group when Space key is pressed', async () => {
      renderWithProviders(<GroupsEditorModal closeOverlay={mockCloseOverlay} />);
      ...
      await userEvent.type(input, 'space-group ');

      expect(screen.getByText('space-group')).toBeInTheDocument();
    });

    it('adds new group when Comma key is pressed', async () => {
      renderWithProviders(<GroupsEditorModal closeOverlay={mockCloseOverlay} />);
      ...
      await userEvent.type(input, 'comma-group,');

      expect(screen.getByText('comma-group')).toBeInTheDocument();
    });
  ...
  describe('Deleting groups', () => {
    ...
    it('deselects group when it is deleted', async () => {
      ...
      expect(screen.getByText('Select a group above')).toBeInTheDocument();
    });
  });

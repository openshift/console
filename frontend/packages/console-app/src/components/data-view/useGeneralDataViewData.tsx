...
import { useDataViewSort, getSortByDirection } from './useDataViewSort';
...
export function useGeneralDataViewData<
...
>({
  ...
  const dataViewColumns = React.useMemo<GeneralDataViewColumn<TData>[]>(
    () =>
  ...
  const { sortBy, onSort } = useDataViewSort<TData>({
  ...
  const transformedData = sortedData
    .map<GeneralRowProps<TData, TCustomRowData>>((obj, index) => ({
      obj,

...
export type GeneralDataViewProps<TData, TCustomRowData, TFilters> = {
  ...
  getDataViewRows: GetGeneralDataViewRows<TData, TCustomRowData>;
...
/**
 * Generic Console DataView component based on PatternFly DataView.
 * This component works with any data type, not just K8s resources.
 */
export const GeneralDataView = <
  TData extends any = any,
  TCustomRowData extends any = any,
...
>({
  ...
  getNameFromItem,
  getLabelsAsString,
}: GeneralDataViewProps<TData, TCustomRowData, TFilters>) => {
  const { t } = useTranslation();

  const { filters, onSetFilters, clearAllFilters, filteredData } = useGeneralDataViewFilters<
    TData,
    TFilters
  >({
    data,
  ...
  const { dataViewColumns, dataViewRows, pagination } = useGeneralDataViewData<
    TData,
    TCustomRowData,
    TFilters
  >({
  ...
  const dataViewFilterNodes = React.useMemo<React.ReactNode[]>(() => {
    ...
    if (!hideNameLabelFilters && getNameFromItem) {
      basicFilters.push(
        <DataViewTextFilter
          key="name"
          filterId="name"
    ...
    if (!hideNameLabelFilters && !hideLabelFilter && loaded && getLabelsAsString) {
      basicFilters.push(
  ...
  }, [additionalFilterNodes, t, loaded, getNameFromItem, getLabelsAsString]);

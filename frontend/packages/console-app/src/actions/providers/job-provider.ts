...
import { useJobActions } from '../creators/job-factory';
import { usePDBActions } from '../creators/pdb-factory';

export const useJobActionsProvider = (resource: JobKind) => {
  const [kindObj, inFlight] = useK8sModel(referenceFor(resource));
  const [pdbActions] = usePDBActions(kindObj, resource);
  const jobActions = useJobActions(kindObj, resource);
  const actions = React.useMemo(
    () => [...jobActions, ...pdbActions, ...getCommonResourceActions(kindObj, resource)],
    [kindObj, resource, pdbActions, jobActions],
  );

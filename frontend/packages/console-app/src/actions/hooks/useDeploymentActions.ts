 ...
 * @param {K8sResourceKind} resource - The specific resource instance for which to generate Deployment actions.
 * @param {DeploymentActionCreator[]} [filterActions] - Optional. If provided, the returned `actions` array will contain
 * only the specified actions. If omitted, it will contain all Deployment actions. In case of invalid `actionCreators`
 ...
 *   const actions = useDeploymentActions(kind, obj, [DeploymentActionCreator.EditDeployment, DeploymentActionCreator.RestartRollout] as const);

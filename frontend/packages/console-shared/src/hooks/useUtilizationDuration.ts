...
import { DEFAULT_DURATION, DEFAULT_DURATION_KEY, DurationKeys } from '../constants';
...
export const useUtilizationDuration: UseUtilizationDuration = (
  ...
  const endDate =
    useSelector<RootState, Date>(({ UI }) => UI.getIn(['utilizationDuration', 'endDate'])) ??
    new Date();
  const selectedKey =
    useSelector<RootState, DurationKeys.OneHour | string>(({ UI }) =>
      UI.getIn(['utilizationDuration', 'selectedKey']),

...
const useDefaultSamples = (): Record<GroupVersionKind, Sample[]> => {
  const { t } = useTranslation('console-shared');
  ...
  return {
    ...
    [referenceForModel(ResourceQuotaModel)]: [
      {
        ...
        description: t('Limit the total amount of memory and CPU that can be used in a namespace.'),
        id: 'rq-compute',
    ...
    [referenceForModel(RoleModel)]: [
      {
        ...
        description: t('This "Role" is allowed to read the resource "Pods" in the core API group.'),
    ...
    [referenceForModel(ClusterRoleModel)]: clusterRoleBindingSamples,
...
export const useResourceSidebarSamples = (
  ...
  const existingSamples = defaultSamples[referenceForModel(kindObj)] || [];

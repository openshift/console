...
export const requesterFilter = (filter, obj): boolean => {
  ...
  if (filter.selected.has(REQUESTER_FILTER.ME) && isCurrentUser(requester)) {
    return true;
  }

  if (filter.selected.has(REQUESTER_FILTER.USER) && isOtherUser(requester, obj.metadata.name)) {
    return true;
  }

  if (
    filter.selected.has(REQUESTER_FILTER.SYSTEM) &&

 ...
 *
 * This module provides the core decision logic for determining which OLS workflow
 * and prompt to use based on cluster state.
 */
...
export const generateUpdatePrompt = (
  ...
  const context: UpdateWorkflowContext = {
    phase,
    cv,
    clusterOperators,
    machineConfigPools,
 ...
 * Re-exported from predicates for convenience
 */
export const hasOperatorIssues = hasAnyOperatorIssues;
...
export const determineWorkflowPhase = (
  ...
  const progressing = isClusterProgressing(cv);
  ...
  if (failing || invalid || retrievedUpdates || releaseAccepted || operatorIssues || progressing) {
    return 'status';
  }
...
/**
 * Extract risk information from conditional updates
 * Parses the conditions to extract human-readable risk descriptions
 */
export const getConditionalUpdateRisks = (
  cv: ClusterVersionKind,
): {
  version: string;
  risks: { reason: string; message: string }[];
}[] => {
  const conditionalUpdates = cv.status?.conditionalUpdates || [];

  return conditionalUpdates.map((update) => ({
    version: update.release.version || 'Unknown',
    risks:
      update.conditions
        ?.filter((c) => c.status === 'False' && c.type === 'Recommended')
        .map((c) => ({
          reason: c.reason || 'UnknownRisk',
          message: c.message || 'No details available',
        })) || [],
  }));
};

...
const mockCodeEditorField = (CodeEditorField as unknown) as jest.Mock;
const mockDynamicFormField = (DynamicFormField as unknown) as jest.Mock;
const mockRadioGroupField = (RadioGroupField as unknown) as jest.Mock;
const mockLoadingBox = (LoadingBox as unknown) as jest.Mock;
...
describe('SyncedEditorField', () => {
  ...
  const props: SyncedEditorFieldProps = {
    ...
    formContext: {
      name: 'formData',
      editor: <div>Form Editor</div>,
      isDisabled: false,
    },
    yamlContext: {
      name: 'yamlData',
      editor: <div>YAML Editor</div>,
      isDisabled: false,
    },
  ...
  beforeEach(() => {
    mockCodeEditorField.mockClear();
    mockDynamicFormField.mockClear();
  ...
  it('should render form editor if useEditorType returns form', () => {
    ...
    const { container } = renderWithProviders(<SyncedEditorField {...props} />);

    expect(container.textContent).toContain('Form Editor');
  });
  ...
  it('should render yaml editor if useEditorType returns yaml', () => {
    ...
    expect(container.textContent).toContain('YAML Editor');
  });

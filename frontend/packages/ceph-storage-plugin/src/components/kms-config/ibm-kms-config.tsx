...
import { advancedIbmKmsModal } from '../modals/advanced-kms-modal/advanced-ibm-kms-modal';
import { IbmKmsConfig, ProviderNames } from '../../types';
import './kms-config.scss';

export const IbmKmsConfigure: React.FC<KMSConfigureProps> = ({ state, dispatch, className }) => {
  const { t } = useTranslation();

  const kms: IbmKmsConfig = state.kms?.[ProviderNames.IBMROKS];
  const kmsObj: IbmKmsConfig = _.cloneDeep(kms);

  React.useEffect(() => {
    const hasHandled: boolean = kmsConfigValidation(kms, ProviderNames.IBMROKS);
    if (kms.hasHandled !== hasHandled)
      dispatch({
        type: 'securityAndNetwork/setIbmKms',
        payload: {
          ...kms,
          hasHandled,
        },

...
import { scReducer, scInitialState } from '../../utils/kms-encryption';
...
export const StorageClassEncryptionKMSID: React.FC<ProvisionerProps> = ({
  ...
  React.useEffect(() => {
    if (csiConfigMapLoaded && !csiConfigMapLoadError && csiConfigMap) {
      setCsiKmsDetails(csiConfigMap);
    }
  ...
  }, [csiConfigMap, csiConfigMapLoaded, csiConfigMapLoadError, setIsExistingKms, setEncryptionId]);
    ...
    if (
      ...
      try {
        ...
        await Promise.all(promises).then(() => {
          ...
          setEncryptionId(state.kms.name.value);
        });
        setErrorMessage('');
      } catch (error) {
                ...
                <ButtonBar errorMessage={errorMessage} inProgress={progress}>
                  <ActionGroup>
                    <Button
                      variant="secondary"
                      onClick={updateKMS}
                      isDisabled={!scKmsConfigValidation(state.kms)}

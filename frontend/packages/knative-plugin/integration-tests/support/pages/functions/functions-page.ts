...
export const functionsPage = {
  ...
  verifyFunctionsActionsDropdown: () =>
    cy.byTestID('console-select-menu-toggle').eq(0).should('be.visible'),
  clickFunctionsActionButton: () => cy.byTestID('console-select-menu-toggle').eq(0).click(),
  verifyActionsInCreateMenu: () => {
    cy.byTestID('console-select-item').contains('Import from Git').should('exist');
    cy.byTestID('console-select-item').contains('Samples').should('exist');
  },

...
import { appHost, testName, retry } from '../../../../integration-tests/protractor.conf';
import {
...
export const installKnOperator = async () => {
...
export const knativeServingYAML = async () => {
  await execSync(`oc new-project knative-serving`);
  await execSync(
    `oc apply -f ./packages/knative-plugin/integration-tests/utils/serverless-service.yaml`,
  );
...
export const increaseMachineSets = async () => {
...
export const checkServerlessInstalled = async () => {
  await crudView.isLoaded();

...
describe(ClusterServiceVersionResourcesPage.displayName, () => {
  ...
  it('passes `createProps` for dropdown create button if app has multiple owned CRDs', () => {
    ...
    expect(listPage.props().createProps.createLink(obj.spec.customresourcedefinitions.owned[0].name)).toEqual(`/k8s/ns/default/${ClusterServiceVersionModel.plural}/testapp/testapp.coreos.com:v1:TestResource/_new`);
  });
  ...
  it('passes `createProps` for single create button if app has only one owned CRD', () => {
    ...
    expect(listPage.props().createProps.to).toEqual(`/k8s/ns/default/${ClusterServiceVersionModel.plural}/testapp/testapp.coreos.com:v1:TestResource/_new`);
  });
